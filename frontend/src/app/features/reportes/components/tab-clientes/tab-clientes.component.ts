...
export class TabClientesComponent implements OnChanges, AfterViewInit, OnDestroy {
  @Input() filtro!: ReporteFiltro;
  @ViewChild('chartZonas') chartZonasRef?: ElementRef<HTMLCanvasElement>;

  data: any = null;
  loading = true;
  private chartZonas?: Chart;
  ...
  private cargar(): void {
    ...
    this.svc.getClientes(this.filtro).pipe(takeUntil(this.destroy$)).subscribe({
      next: d => {
  ...
  private dibujar(): void {
    this.zone.runOutsideAngular(() => this.dibujarZonas());
  }

  private dibujarZonas(): void {
    if (!this.chartZonasRef?.nativeElement || !this.data?.por_zona) return;
    this.chartZonas?.destroy();
    const colores = ['#E11D48', '#7C3AED', '#0891B2', '#D97706', '#059669', '#6366F1', '#EC4899'];
    this.chartZonas = new Chart(this.chartZonasRef.nativeElement, {
      type: 'doughnut',
      data: {
        labels: this.data.por_zona.map((z: any) => z.zona),
        datasets: [{
          data: this.data.por_zona.map((z: any) => z.cantidad),
