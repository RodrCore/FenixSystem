// prisma/seed-productos.ts
// Seed completo: Categorias, Presentaciones, Proveedores, Productos, ProductoPresentacion, Lotes
...
// ══════════════════════════════════════════════════════
// DATOS DEL TSV — nombre limpio + precio
// ══════════════════════════════════════════════════════
...
// ── Función para inferir categoría según nombre ──────────────────────────────
function inferirCategoria(nombre: string): string {
  const n = nombre.toUpperCase();
  if (/CAFE|NESCAFE|COPACABANA/.test(n)) return 'Café e Infusiones';
  if (/API|SOPA|CHAIRITO|LAGUA|VIANDADA/.test(n)) return 'Alimentos Preparados';
  if (/COCOA|CHOCOLATE DE COCINA|COBERTURA|PASTA DE CACAO/.test(n)) return 'Ingredientes y Repostería';
  if (/TABLETA|CHOCOLATE/.test(n)) return 'Tabletas de Chocolate';
  if (/GALLETA|CLUB SOCIAL|CHIP AHOY|RITZ|OREO|FRANCESA|TODDY GALLETA|FRAC|GALLETON/.test(n)) return 'Galletas y Bizcochos';
  if (/CHICLET|BELDENT|TRIDENT|TOPLINE|CLORETS|HALLS|MINTY|BUBALOO|GROSSO|BAZOOKA|CENTRO|ACIDO|MASTICABLE|MENTA|GELATIN|GOMA/.test(n)) return 'Chicles y Gomas';
  if (/CARAMELO|CHUPETE|TENTACION|LOTE|AVENTURA|POOSH|PICADILLO|POP KISS|SPARKIES|LUX|RESERVADA|SUAVESITO|DINAMICO|CHOCLITO|OKA/.test(n)) return 'Caramelos y Dulces';
  if (/ALFAJOR|BON O BON|BATON|NIKOLO|VIZZIO|DOBLON|KILATE|KILOMBO|KRAPUL|MECANO|MOMENTO|GOLPE|GOLAZO|GAROTO|GENIO|DONOFRIO|NUCITA|CHUBI/.test(n)) return 'Alfajores y Bombones';
  if (/GRAGEA|MASMELO|MIEL|TOFFEE|TOFFE|TURRON|MANJAR|RELLENO WATTS/.test(n)) return 'Confitería Especial';
  if (/PANATON|PANETON|PANETONE|PARIS|BODAS|PASCUERA|JO JO|CHRISTMAS|PIAZZA|FRANCESA/.test(n)) return 'Panetones y Repostería';
  if (/HUEVO|CONEJO|BENNY|NUEZ DE PASCUA|DINOVOS/.test(n)) return 'Temporada y Especiales';
  if (/FILIPITO|FIDOS|SPACE ARROZ|SPACE FIDEO|SPACE QUINUA|ARROZ DE BREICK|AMARANTO|PALITOS/.test(n)) return 'Snacks y Cereales';
  if (/BUUH|YUMMY|AGOGO|BARRILETE|BURBUJITAS|PRIVILEGIO|ANGELITO|AMBROSITO|OLE OLE|ACONCAGUA|BON NUIT|BRUSOLITO/.test(n)) return 'Golosinas Infantiles';
  if (/CEIBO|CEIBOLITO|TE AMO|TE QUIERO|BOMBONES|SELECTO|INTENSO|SEDUCCION|DULCE ENCANTO|CORAZON|EMOTICON|EMBARE/.test(n)) return 'Chocolates Bolivianos Ceibo';
  if (/TODDY|COCOA/.test(n)) return 'Polvos de Cacao y Malteadas';
  if (/LECHE GLORIA|NESTLE/.test(n)) return 'Lácteos';
  if (/LIDITA|ATUN/.test(n)) return 'Conservas';
  if (/KLEENEX/.test(n)) return 'Higiene Personal';
  if (/YOGUETA|BIOOPS|CHOCMAN|DUCREM/.test(n)) return 'Chocolates y Obleas';
  return 'Otros';
}

// ── Función para inferir marca ───────────────────────────────────────────────
function inferirMarca(nombre: string): string {
  const n = nombre.toUpperCase();
  if (/CEIBO|CEIBOLITO/.test(n)) return 'Ceibo';
  if (/ARCOR|BAZOOKA|BON O BON|BELDENT|HALLS|GOLPE|GAROTO|GENIO|GROSSO|MASTICABLE ARCOR|MIEL DE ARCOR/.test(n)) return 'Arcor';
  if (/NESTLE|NESCAFE|LECHE GLORIA|MILO|KIT KAT|OREO/.test(n)) return 'Nestlé';
  if (/OREO|CHIPS AHOY|RITZ|CLUB SOCIAL|BELDENT/.test(n)) return 'Mondelez';
  if (/FRANCESA/.test(n)) return 'Francesa';
  if (/BREICK|POPS DE BREICK|COCOA INCADEX|GALLETON BREICK|ARROZ DE BREICK|TABLETA.*BREICK/.test(n)) return 'Breick';
  if (/COPACABANA/.test(n)) return 'Copacabana';
  if (/AMBROSOLI/.test(n)) return 'Ambrosoli';
  if (/WATTS|TOFFE EN BOLSA WATTS|RELLENO WATTS/.test(n)) return 'Watt\'s';
  if (/FERRARI/.test(n)) return 'Ferrari Ghezzi';
  if (/TODDY|VIANDADA/.test(n)) return 'Toddy';
  return 'Genérica';
}
...
async function main() {
  ...
  try {
    ...
    const presentacionMap: Record<string, number> = {};
    ...
    // ════════════════════════════════════════════════════════
    // 3. PROVEEDOR (1 genérico para todos)
    // ════════════════════════════════════════════════════════
    ...
    const proveedores = [
      {
        ...
        latitud:            new Decimal('-19.5836'),
        longitud:           new Decimal('-65.7531'),
      ...
      },
      {
        ...
        latitud:            new Decimal('-19.5900'),
        longitud:           new Decimal('-65.7600'),
    ...
    // ════════════════════════════════════════════════════════
    // 4. PRODUCTOS + PRODUCTO_PRESENTACION + LOTE
    // ════════════════════════════════════════════════════════
    ...
    fechaVencimiento.setFullYear(hoy.getFullYear() + 1); // vence en 1 año
    ...
    for (let i = 0; i < rawProductos.length; i++) {
      ...
      const precioVenta = new Decimal(raw.precio.toFixed(2));
      const precioCompra = precioVenta.times(new Decimal('0.72')).toDecimalPlaces(2);
      const margen = new Decimal('28.00');
      ...
      const producto = await prisma.producto.upsert({
        ...
        create: {
          ...
          iva_porcentaje: new Decimal('0'),
          ieps_porcentaje: new Decimal('0'),
          stock_minimo: 10,
          stock_maximo: 500,
      ...
      // ─ Crear ProductoPresentacion (UNIDAD) ───────────────
      const ppUnidad = await prisma.productoPresentacion.upsert({
        ...
        create: {
          producto_id: producto.id,
          presentacion_id: presentacionMap['UNIDAD'],
          unidades_equivalentes: new Decimal('1'),
          precio_venta: precioVenta,
          precio_mayoreo: precioVenta.times(new Decimal('0.95')).toDecimalPlaces(2),
          cantidad_minima_mayoreo: 12,
          activo: true,
        },
      });

      // ─ Crear ProductoPresentacion (CAJA = 12 unidades) ───
      await prisma.productoPresentacion.upsert({
        ...
        create: {
          producto_id: producto.id,
          presentacion_id: presentacionMap['CAJA'],
          unidades_equivalentes: new Decimal('12'),
          precio_venta: precioVenta.times(new Decimal('12')).toDecimalPlaces(2),
          precio_mayoreo: precioVenta.times(new Decimal('12')).times(new Decimal('0.92')).toDecimalPlaces(2),
          cantidad_minima_mayoreo: 3,
          activo: true,
        },
      });

      // ─ Crear Lote inicial (stock de 100 cajas) ───────────
      ...
      await prisma.lote.upsert({
        ...
        create: {
          producto_id:                   producto.id,
          proveedor_id:                  proveedorIds[i % 2], // alterna entre 2 proveedores
          codigo_lote:                   codigoLote,
          fecha_ingreso:                 hoy,
          ...
          cantidad_unidades_inicial:     1200,  // 100 cajas × 12 unidades
          cantidad_unidades_disponible:  1200,
          unidades_por_presentacion:     12,
          costo_unitario:                precioCompra,
          costo_total:                   precioCompra.times(new Decimal('1200')).toDecimalPlaces(2),
          estado:                        'Disponible',
