...
var BoletasService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.BoletasService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../prisma/prisma.service");
const pdfkit_1 = __importDefault(require("pdfkit"));
let BoletasService = BoletasService_1 = class BoletasService {
    prisma;
    logger = new common_1.Logger(BoletasService_1.name);
    constructor(prisma) {
        this.prisma = prisma;
    }
    async generarBoletaPDF(pedidoId, usuarioId, rolNombre) {
        ...
        if (!esAdmin && rolNombre === 'PREVENTISTA') {
            if (pedido.preventista_id !== usuarioId) {
                throw new common_1.ForbiddenException('No puedes generar boleta de pedidos ajenos');
            }
        }
        if (rolNombre === 'REPARTIDOR') {
            throw new common_1.ForbiddenException('El repartidor no puede generar boletas');
        }
        ...
        if (pedido.cliente.direccion_calle) {
            doc.text(`${pedido.cliente.direccion_calle} ${pedido.cliente.direccion_numero ?? ''}`, 50, y);
            y += 12;
        }
        ...
        if (pedido.preventista) {
            doc
                ...
                .font('Helvetica-Bold')
                .fontSize(11)
                .text(`${pedido.preventista.nombres} ${pedido.preventista.apellido_paterno ?? ''}`, 350, yDerecha);
            yDerecha += 14;
        }
        doc
            .fillColor('#94A3B8')
        ...
        for (const d of pedido.detalles) {
            doc
                ...
                .text(`Bs ${Number(d.precio_unitario_presentacion).toFixed(2)}`, 390, y, { width: 60, align: 'right' })
        ...
        if (Number(pedido.descuento_general) > 0) {
            doc
                ...
                .fillColor('#DC2626')
                .font('Helvetica-Bold')
                .text(`- Bs ${Number(pedido.descuento_general).toFixed(2)}`, xLabel, y, { width: 165, align: 'right' });
            y += lineH;
        }
        ...
        doc
            ...
            .font('Helvetica')
            .fontSize(8)
            .text(`Documento generado el ${new Date().toLocaleString('es-BO')} · Fenix · Sistema de gestión`, 50, footerY + 8, { width: 495, align: 'center' });
        doc.end();
...
exports.BoletasService = BoletasService;
exports.BoletasService = BoletasService = BoletasService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], BoletasService);
//# sourceMappingURL=boletas.service.js.map